...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['category_name'])) {
        $valid = 0;
        $error_message .= "Kategori Başlığı boş olamaz<br>";
    } else {
    	...
    	if($total) {
    		$valid = 0;
        	$error_message .= 'Kategori Başlığı zaten var<br>';
    	}
    }
    if($valid == 1) {
		...
		$statement = $pdo->prepare("UPDATE tbl_category SET category_name=?, category_slug=?, meta_title=?, meta_description=? WHERE category_id=?");
		$statement->execute(array($_POST['category_name'],$category_slug,$_POST['meta_title'],$_POST['meta_description'],$_REQUEST['id']));
...
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="category.php" class="btn btn-primary btn-sm">Kategorileri Listele</a>
	</div>
</section>
<?php							
foreach ($result as $row) {
        ...
        <div class="box box-info">
            <div class="box-body">
                <div class="form-group">
                    <label for="" class="col-sm-2 control-label">Kategori Başlığı <span>*</span></label>
                    <div class="col-sm-4">
